...
def get_conversations(patient_id: str) -> list[dict]:
    ...
    rows = conn.execute(
        ...
        "(SELECT content FROM chat_messages m2 WHERE m2.conversation_id = chat_messages.conversation_id "
        " AND m2.patient_id = chat_messages.patient_id AND m2.role = 'user' ORDER BY m2.created_at LIMIT 1) as title, "
        "(SELECT content FROM chat_messages m3 WHERE m3.conversation_id = chat_messages.conversation_id "
        " AND m3.patient_id = chat_messages.patient_id ORDER BY m3.created_at DESC LIMIT 1) as last_message "
        "FROM chat_messages WHERE patient_id = ? GROUP BY conversation_id ORDER BY last_message_at DESC",
        (patient_id,),
    ).fetchall()
    conn.close()
