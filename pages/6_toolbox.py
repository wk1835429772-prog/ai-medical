...
# 收藏区
if st.session_state.calc_favorites:
    fav_calcs = [c for c in calculators if c["id"] in st.session_state.calc_favorites]
    if fav_calcs and not search:
        st.subheader("⭐ 收藏")
        cols = st.columns(min(6, len(fav_calcs)))
...
def _render_calc_card(calc):
    ...
    with st.container():
        ...
        with col1:
            st.subheader(f"{calc['icon']} {calc['name']}")
        with col2:
            col_fav, col_close = st.columns(2)
            with col_fav:
                is_fav = calc_id in st.session_state.calc_favorites
                if st.button("⭐ 取消收藏" if is_fav else "☆ 收藏", key=f"fav_toggle_{calc_id}"):
                    if is_fav:
                    ...
                    st.rerun()
            with col_close:
                if st.button("✖ 关闭", key=f"close_{calc_id}"):
                    st.session_state[f"open_calc_{calc_id}"] = False
                    st.rerun()

        # 输入区
        inputs = calc.get("inputs", [])
        values = {}
        cols = st.columns(min(4, len(inputs)))
            ...
            with cols[i % 4]:
                ...
                if inp.get("type") == "check":
                    ...
                    values[k] = st.number_input(
                        label, step=step, format=f"%.{len(str(step).split('.')[-1])}f" if '.' in str(step) else "%g",
                        key=f"inp_{calc_id}_{k}", value=None
                    )

        # 计算按钮
        if st.button("🧮 计算", key=f"compute_{calc_id}", type="primary"):
...
def _run_calc(calc_id, values):
    ...
    try:
        ...
        return {"main": f"计算错误", "detail": str(e)}
